...
| 33–46 | 28–36 | **Act 3 — the standards, one per question.** *Say it once*: the rulebook as the diff they will read, three rules as green added lines (+ live: add them, commit) · *Don't take its word*: the sentence next to the verdict, and what makes a check a judge (+ live: run the red tests on stage) · *Red is a to-do list*: the failing test read bottom up by number · *Send the failure back*: the loop with the tests in it · *All three, in one message*: the ask as the message they will paste, the two slots that change highlighted, each paragraph labelled beside with its part and its reason · Ralph, three slides (who he is · the loop · the signs) · `/goal`. | Every laptop: an `AGENTS.md` commit pushed |
