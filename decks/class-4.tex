...
The model never saw the file — it had to ask. And the approval prompt is where you sit inside the
...
\hue{NovaPurple}
\begin{frame}{What actually goes back and forth.}
...
\end{tabular}
\vskip6mm
\muted{It never saw the file. It asked; you allowed it; the file's own line was pasted into the
conversation. That line is the only reason it can answer.}
\vfill
\end{frame}
...
buys more computation and gives the model a scratchpad it can read back — which is why "think step
by step" is not a politeness trick. Lineage: Nye et al. 2021 called it a scratchpad; Wei et al.
...
screen, which is exactly why you still read the diff.}
...
% ── 10 · the long run-up
\hue{NovaPurple}
\begin{frame}{The ideas came first. By decades.}
\framesubtitle{How we got here · the long run-up}
\diagramtight[0.82\linewidth]{figures/prehistory.pdf}
\vskip4mm
\muted{Sixty years of theory. \textbf{What was missing was never the idea.}}
\vfill
\end{frame}
...
XOR — about as simple as a function gets. Funding collapsed; the first AI winter. Careful with the
...
\vspace*{\fill}
\begin{tcbitemize}[cardsS, raster columns=2]
  \carditem{Compute}{Arithmetic got cheap}{2007 — graphics cards became programmable.
    2012 — a network trained on two of them won the image contest outright.}
  \carditem{Data}{The web became a corpus}{From 2007 — a non-profit crawling the public web and
    giving it away. Billions of pages of text, free.}
\end{tcbitemize}
\vskip6mm
...
\vfill
\end{frame}
\tanote{This is the payoff of the previous slide, and the line to land: the ideas sat there for
...
arithmetic units built for game pixels and, as it turns out, exactly what a neural network needs.
...
about intelligence. If someone asks why this happened now and not in 1990, that is the whole
answer.}
...
eight get their own because this is where the job changed, not just the size. Say the four band
names as you go — completing,
...
\emph{model\_v3\_FINAL\_jm}. Everybody's copy starts from here.}
\vfill
\end{frame}
...
\vskip6mm
\muted{Three commits deep, half of it broken, the agent having a bad day — the version everyone
else is using is exactly as you left it.}
\vfill
\end{frame}
...
\vskip1mm
\muted{\textbf{Push} puts your draft where the team can see it. A \textbf{pull request} is you
asking someone to read the diff before it counts — today, that someone is you, reviewing the agent.}
\end{frame}

% ── git 6 · merge
\hue{NovaBlue}
\begin{frame}{Approved. Now it is the official version.}
\framesubtitle{Git · the sign-off}
\vspace*{\fill}
\gitstory{figures/git-story-5.pdf}
\vskip6mm
\muted{\textbf{Merge} is the sign-off. \code{main} now includes your work — and the next person's
copy starts from here.}
\vfill
\end{frame}
\tanote{Land the cycle: they are now back at slide one, with a bigger main. That is the whole
