...
% ── context rot
% Same bar as the slide before, twice: the wash over the middle is the published "lost in the
% middle" shape drawn on the object they already know, and the second bar is what compaction does.
\hue{NovaPurple}
...
\contextrot
\vskip3mm
\muted{It reads the ends well and the middle badly. Windows grew a hundredfold; that did not
change.}
\vskip1mm
{\tiny\color{NovaMuted}Liu et al.\ (2024), \emph{Lost in the Middle}, TACL 12 ·
Chroma (2025), \emph{Context Rot}: 18 models, every one worse as the input grew}
\vfill
\end{frame}
\tanote{Same bar as the previous slide, so point at it: an hour in, it is full, and the wash over the
middle is how well the model reads each part. The pin is the point — the error they pasted is in
the soft part. Windows went 8k, 32k, 128k, a million; the number on the box grew a hundredfold
and the wash did not move. Liu et al.\ measured the U shape in 2024 and it has held
